...
(function($){
    ...
    var AdminAPP = {
        initialize: function() {
	        ...
	        AdminAPP.header_layout_hover();
            AdminAPP.initTaxMetaIconSelect();
        },
	    ...
	    header_layout_hover: function() {
		    $('#g5plus_megatron_options-header_layout li.redux-image-select').on('hover', function (event) {
			    var $this = $(this);
			    var $tr = $(this).closest('tr');
			    if ($tr.length == 0) {
				    return;
			    }
			    var url_header = $('label > img', this).attr('src');
			    var header_value = $('label > input', this).attr('value');
			    var url_header_hover = url_header.replace(header_value + '.png', header_value + '-hover.png');
			    if (event.type=='mouseenter') {
					if ($('.header-layout-hover-wrapper').length == 0) {
						$($tr).after('<tr class="header-layout-hover-wrapper"><td colspan="2"><img src="' + url_header_hover + '"/></td></tr>');
					}
				    $('.header-layout-hover-wrapper img').attr('src', url_header_hover);
				    $('.header-layout-hover-wrapper').show();
			    }
			    else {
				    $('.header-layout-hover-wrapper').hide();
			    }
		    });
	    },
        initTaxMetaIconSelect:function(){
            if(jQuery.isFunction(jQuery.fn.select2)){
                $(".at-select").select2({
                    formatResult: AdminAPP.formatIconState,
                    formatSelection: AdminAPP.formatIconState
                });
            }
        },
