...
abstract class RWMB_Field {
	...
	public static function show( $field, $saved, $post_id = 0 ) {
		...
		$required_field = '';
		if (!empty($field['required-field']) && is_array($field['required-field'])) {
			$required_field = " data-required='" . json_encode($field['required-field']) . "'";
		}

		$outer_html = sprintf(
			$field['before'] . '<div %s class="%s">%s</div>' . $field['after'],
			$required_field,
			trim( $classes ),
			$html
		);
		...
		echo sprintf('%s',$outer_html); // WPCS: XSS OK.
	}
	...
	public static function normalize( $field ) {
		$field = wp_parse_args( $field, array(
			...
			'max_clone'  => 0,
			'sort_clone' => false,
			'add_button' => __( '+ Add more', 'g5plus-academia' ),
