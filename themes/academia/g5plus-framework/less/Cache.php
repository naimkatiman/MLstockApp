...
require_once( G5PLUS_THEME_DIR . 'g5plus-framework/less/Version.php');
...
class Less_Cache{
			...
			if( file_exists($list_file) ){
				...
				if( !$cached_name || $cached_name === $compiled_name ){
					...
					if( $output_file && file_exists($output_file) ){
						@touch($list_file);
						return wp_basename($output_file); // for backwards compatibility, we just return the name of the file
					}
				}
			}
		}
	...
	public static function Cache( &$less_files, $parser_options = array() ){
		...
		$file = G5PLUS_THEME_DIR . 'g5plus-framework/less/Less.php';
	...
	public static function CheckCacheDir(){
		...
		if( !file_exists(Less_Cache::$cache_dir) ){
			if( !wp_mkdir_p(Less_Cache::$cache_dir) ){
