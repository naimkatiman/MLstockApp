...
class scss_formatter {
	...
	protected function block($block) {
		...
		if (!empty($block->lines)) {
			...
			echo sprintf('%s', $inner) . implode($glue, $block->lines);
			if (!empty($block->children)) {
				echo sprintf('%s', $this->break);
			}
		}
		...
		if (!empty($block->selectors)) {
			$this->indentLevel--;
			if (empty($block->children)) echo sprintf('%s', $this->break);
			echo sprintf('%s', $pre . $this->close . $this->break);
		}
	}
...
class scss_server {
	...
	public function serve($salt = '') {
		...
		if ($input = $this->findInput()) {
			...
			echo sprintf('%s', $wp_filesystem->get_contents($output));
	...
	public function __construct($dir, $cacheDir=null, $scss=null) {
		...
		if (!is_dir($this->cacheDir)) wp_mkdir_p($this->cacheDir, 0755, true);
