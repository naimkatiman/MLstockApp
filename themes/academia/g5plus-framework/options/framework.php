...
// Fix for the GT3 page builder: http://www.gt3themes.com/wordpress-gt3-page-builder-plugin/
    ...
    if ( ! class_exists( 'ReduxFrameworkInstances' ) ) {
        // Instance Container
        require_once(G5PLUS_THEME_DIR . 'g5plus-framework/options/inc/class.redux_instances.php');
        require_once(G5PLUS_THEME_DIR . 'g5plus-framework/options/inc/lib.redux_instances.php');
    }
...
// Don't duplicate me!
    if ( ! class_exists( 'ReduxFramework' ) ) {
        ...
        // Redux API class  :)
        require_once(G5PLUS_THEME_DIR . 'g5plus-framework/options/inc/class.redux_api.php');

        // General helper functions
        require_once(G5PLUS_THEME_DIR . 'g5plus-framework/options/inc/class.redux_helpers.php');

        // General functions
        require_once(G5PLUS_THEME_DIR . 'g5plus-framework/options/inc/class.redux_functions.php');
        require_once(G5PLUS_THEME_DIR . 'g5plus-framework/options/inc/class.p.php');

        require_once(G5PLUS_THEME_DIR . 'g5plus-framework/options/inc/class.redux_filesystem.php');

        // ThemeCheck checks
        require_once(G5PLUS_THEME_DIR . 'g5plus-framework/options/inc/class.redux_themecheck.php');
        ...
        class ReduxFramework {
            ...
            public static $_version = '3.5.4.13';
            public static $_dir;
            public static $_url;
            ...
            public static function init() {
                $dir = Redux_Helpers::cleanFilePath( G5PLUS_THEME_DIR . 'g5plus-framework/options' );
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( esc_html__( 'Options panel created using %1$s', 'redux-framework' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . esc_html__( 'Redux Framework', 'redux-framework' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = esc_html__( 'Options', 'redux-framework' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = esc_html__( 'Options', 'redux-framework' );
                }
...
// get_instance()
            ...
            private function _tracking() {
                require_once(G5PLUS_THEME_DIR . 'g5plus-framework/options/inc/tracking.php');
...
// get_wordpress_data()
            ...
            public function show( $opt_name, $default = '' ) {
                ...
                if ( ! is_array( $option ) && $option != '' ) {
                    ...
                    echo sprintf('%s', $this->_get_default( $opt_name, $default ));
                }
            }
// show()
            ...
            private function add_submenu( $page_parent, $page_title, $menu_title, $page_permissions, $page_slug ) {
                ...
                if ( isset ( $test[ $page_parent ] ) ) {
                    ...
                    if ( true == $addMenu ) {
	                    $this->page = call_user_func('add_' . 'submenu_page',
            ...
            public function _options_page() {
                ...
                if ( $this->args['menu_type'] == 'hidden' ) {
	                ...
	                $this->page = call_user_func('add_' . 'menu_page',
                    ...
                    if ( true === $this->args['allow_sub_menu'] ) {
                        if ( ! isset ( $section['type'] ) || $section['type'] != 'divide' ) {
                            foreach ( $this->sections as $k => $section ) {
	                            ...
	                            call_user_func('add_' . 'submenu_page',
                        ...
                        if ( true == $this->args['system_info'] ) {
	                        call_user_func('add_' . 'submenu_page',
                                $this->args['page_slug'], esc_html__( 'System Info', 'redux-framework' ), esc_html__( 'System Info', 'redux-framework' ), $this->args['page_permissions'], $this->args['page_slug'] . '&tab=system_info_default', '__return_null'
                            );
                        }
                    }
                }
                ...
                if ( ! empty ( $this->typography ) && ! empty ( $this->typography ) && filter_var( $this->args['output'], FILTER_VALIDATE_BOOLEAN ) ) {
                    ...
                    if ( $this->args['async_typography'] && ! empty ( $this->typography ) ) {
                        ...
                        <script>
                            ...
                            WebFontConfig['google'] = {families: [<?php echo sprintf('%s', $typography->makeGoogleWebfontString ( $this->typography )) ?>]};
...
// _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    // Construct message
                    $msg = sprintf( esc_html__( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'redux-framework' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;
...
// _register_settings()
            ...
            private function _register_extensions() {
                $path    = G5PLUS_THEME_DIR . 'g5plus-framework/options/inc/extensions/';
...
//                }
            ...
            public function ajax_save() {
                ...
                if ($is_generate_less_to_css) {
                    ...
                    require_once G5PLUS_THEME_DIR . 'g5plus-framework/core/generate-less.php';
