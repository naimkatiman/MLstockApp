...
class Tax_Meta_Class {
  ...
  public function __construct ( $meta_box ) {
    ...
    $this->SelfPath = get_template_directory_uri() . '/g5plus-framework/tax-meta-class';
  ...
  public function check_field_time() {
    ...
    if ( $this->has_field( 'time' ) && $this->is_edit_page() ) {
      ...
      // Enqueu JQuery UI, use proper version.
      $this->enqueue_jqueryui();
      wp_enqueue_script( 'at-timepicker', $plugin_path . '/js/jquery-ui-timepicker-addon.min.js', array( 'tmc-jquery-ui' ),false,true );
    }
  }
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $c = 0;
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        echo '" alt="'.esc_html__('Remove','g5plus-academia').'" title="'.esc_html__('Remove','g5plus-academia').'" id="remove-'.$field['id'].'"></div>';
        $c = $c + 1;
    ...
    echo '" alt="'.esc_html__('Add','g5plus-academia').'" title="'.esc_html__('Add','g5plus-academia').'" id="add-'.$field['id'].'"><br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px;}
  ...
  public function show_field_radio( $field, $meta ) {
	  ...
	  if (empty($meta) && ($meta != 0) && ($meta != '0')) {
		  $meta = $std;
	  }
    $this->show_field_begin( $field, $meta );
      foreach ( $field['options'] as $key => $value ) {
        echo "<input style='{$field['style']}' type='radio' class='at-radio' name='{$field['id']}' value='{$key}'" . checked(  $meta  , $key , false ) . " /> <span class='at-radio-label'>{$value}</span>";
      }
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_image( $field, $meta ) {
    ...
    echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
    if ($has_image)
    echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='".esc_html__('Remove Image', 'g5plus-academia')."' type='button'/>";
    else
    echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='".esc_html__('Upload Image', 'g5plus-academia')."' type='button'/>";
    $this->show_field_end( $field, $meta );
  }
  ...
  public function show_field_taxonomy($field, $meta) {
    ...
    $this->show_field_begin($field, $meta);
    $options = $field['options'];
	$terms = get_categories(array_merge( $options['args'],array('taxonomy' => $options['taxonomy'] )));
  ...
  public function save( $term_id ) {
    ...
    || ( ! check_admin_referer( wp_basename( __FILE__ ), 'tax_meta_class_nonce') )    // Check nonce - Security
  ...
  public function enqueue_jqueryui(){
    ...
    wp_enqueue_style( 'tmc-jquery-ui-css', $plugin_path . '/css/jquery-ui.css');
    wp_enqueue_script( 'tmc-jquery-ui', $plugin_path . '/js/jquery-ui.min.js', array( 'jquery' ) );
  }
  ...
  public function addRadio($id,$options,$args,$repeater=false){
    $new_field = array('type' => 'radio','id'=> $id,'std' => '','desc' => '','style' =>'','name' => 'Radio Field','options' => $options,'multiple' => false);
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
  ...
  public function addImage($id,$args,$repeater=false){
    $new_field = array('type' => 'image','id'=> $id,'desc' => '','style' =>'','name' => 'Image Field','multiple' => false);
    $new_field = array_merge($new_field, $args);
    if(false === $repeater){
      $this->_fields[] = $new_field;
    }else{
  ...
  public function delete_taxonomy_metadata($term,$term_id) {
    ...
    if ( function_exists( 'delete_metadata') ){
        delete_metadata( 'term', $term_id, '', '',true );
    }
  }
  ...
  public function footer_js(){
    ...
    jQuery(document).ready(function(){
      ...
      function clear_form_meta(){
          ...
          jQuery(".mupload_img_holder").next().next().next().val("<?php esc_html_e('Upload Image','g5plus-academia'); ?>");
