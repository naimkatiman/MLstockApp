...
if (!in_array($sidebar, array('left','right'))) {
    ...
    if (($sidebar === false) ||($sidebar === '') || ($sidebar == '-1')) {
...
if ($content_col_number < 12) {
    ...
    $g5plus_woocommerce_single['has_sidebar'] = true;
}
get_header( 'shop' ); ?>
    ...
    <?php endif;?>
        ...
        <?php endif;?>
            <?php while ( have_posts() ) : the_post(); ?>
                <div class="col-md-12"><?php  wc_get_template( 'single-product/title.php' ); ?></div>
                ...
                <div class="site-meta-single-product <?php echo esc_attr($sidebar_col) ?>">
                    <?php wc_get_template_part( 'content', 'single-product-meta-side' ); ?>
                </div>

            <?php endwhile; // end of the loop. ?>
            <?php g5plus_woocommerce_reset_single(); ?>
