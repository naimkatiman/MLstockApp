jQuery(document).ready(function ($){
    var acs_action = 'univero_autocomplete_search';
    $("input[name=s]").autocomplete({
        source: function(req, response){
        	$.ajax({
                url: univero_ajax.ajaxurl+'?callback=?&action='+acs_action,
                dataType: "json",
                data: {
                    term : req.term,
                    category : $(".ninzio-search-form .dropdown_product_cat").val(),
                    post_type : $(".ninzio-search-form .post_type").val()
                },
